...
class CDDRecModel(nn.Module): 
    def __init__(self,args):
        super(CDDRecModel, self).__init__()

        

        self.time_embeddings = nn.Embedding(args.T, args.hidden_size) 
        self.times_embeddings = nn.Embedding(48, args.hidden_size)
        self.time_embed_model = OriginTime2Vec('sin', int(0.5 * args.hidden_size))
        self.time_embed_model_tgt = OriginTime2Vec('sin', int(0.5 * args.hidden_size))
    ...
    def add_position_embedding(self, input_ids, times_ids):
        ...
        if False:
            times_embeddings = self.times_embeddings(times_ids)
        else:
            times_embeddings = self.time_embed_model(times_ids)
        # sequence_emb = item_embeddings + position_embeddings + times_embeddings
        sequence_emb = item_embeddings + position_embeddings
        sequence_emb = rotate(sequence_emb, times_embeddings, 0.5 * self.args.hidden_size, self.args.device)

        # sequence_emb = self.FNN1(sequence_emb)
    ...
    def forward(self, input_ids, times_ids, target_pos, target_neg, aug_input_ids, aug_times_ids, target_times, epoch): # forward
        # input_ids: b*max_Sq



        input_emb, extended_attention_mask = self.add_position_embedding(input_ids, times_ids)
        ...
        if False:
            target_times_embeddings = self.times_embeddings(target_times)
        else:
            target_times_embeddings = self.time_embed_model_tgt(target_times)

        aug_input_emb, aug_extended_attention_mask = self.add_position_embedding(aug_input_ids, aug_times_ids)
        ...
        # x_0 = x_0 + target_times_embeddings
        x_0 = rotate(x_0, target_times_embeddings, 0.5 * self.args.hidden_size, self.args.device)
        # x_0 = self.FNN2(x_0)
    ...
    def inference(self, input_ids, times_ids):
...
def t2v(tau, f, out_features, w, b, w0, b0, arg=None):
    ...
    if arg:
        v1 = f(torch.matmul(tau, w).permute(0, 2, 1) + b, arg)
    else:
        v1 = f(torch.matmul(tau, w).permute(0, 2, 1) + b)
    v2 = torch.matmul(tau, w0).permute(0, 2, 1) + b0
    return torch.cat([v1, v2], 1).permute(2, 0, 1)
...
class SineActivation(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.w0 = nn.parameter.Parameter(torch.randn(in_features, 1, 1))
        self.b0 = nn.parameter.Parameter(torch.randn(in_features, 1, 1))
        self.w = nn.parameter.Parameter(torch.randn(in_features, 1, out_features - 1))
        self.b = nn.parameter.Parameter(torch.randn(in_features, out_features - 1, 1))
        self.f = torch.sin
